...
if ($heure == "tous") {
    ...
    if ($heure_ex[1] == 30) {
        $heure1 = $heure_ex[0] . ":00";
        $heure2 = $h2 . ":00";
    } else {
        $heure1 = $h1 . ":30";
        $heure2 = $heure_ex[0] . ":30";
    }
}

$retour = mysqli_query($connexion, "SELECT COUNT(*) AS nbre_entrees FROM trajets WHERE ((heure >= '$heure1' AND heure <= '$heure2') OR heure='variable') AND ville1 LIKE '%$ville1%' AND ville2 LIKE '%$ville2%' ");
$donnees_compt = mysqli_fetch_array($retour);
$i = $donnees_compt['nbre_entrees'];

$reponse = mysqli_query($connexion, "SELECT * FROM trajets WHERE ((heure >= '$heure1' AND heure <= '$heure2') OR heure='variable') AND ville1 LIKE '%$ville1%' AND ville2 LIKE '%$ville2%'  ") or die(mysqli_error($connexion));
